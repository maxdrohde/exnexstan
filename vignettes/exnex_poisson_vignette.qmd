---
title: "EXNEX with varying follow-up times"
subtitle: ""
author:
...
\end{align*}
```
Below is the Stan code used for the analysis. Note that because Stan cannot handle discrete variables due to the Hamiltonian MCMC used for sampling, the indicator $Z_{j}$ for EX vs NEX must be marginalized out.
...
data {
  ...
  array[J] real<lower=0, upper=1> p_exch; // Mixture probability of exchangeability
...
For the exchangeable priors (mu and tau), we compute the induced prior for the rate parameter by first sampling mu and tau, and then drawing a theta from that distribution. We repeat the process as in the NEX case and observe that these parameters induce a similar prior.
...
We now fit the EX, EXNEX, and NEX models to these data to compare how they perform.
