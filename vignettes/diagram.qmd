...
\node [def, node distance = 6cm, label={\small Definitions}, left of = likelihood, yshift=-2cm] (def) {\begin{tabular}{l} $r_j$: Number of responses \\ $\pi_j$: Response probability \\ $n_j$: Number of participants \\ $p_j$: Prior exchangeability probability \end{tabular}};
...
\node [block, node distance = 4.5cm, label={\small NEX}, below right of = theta] (nex) {$\theta_j \sim \operatorname{Normal}(\mu_0, \tau_0)$};
...
\node [def, node distance = 6cm, label={\small Definitions}, left of = likelihood, yshift=-0.5cm, xshift=-1cm] (def) {\begin{tabular}{l} $\theta_j$: Adverse event rate (log-scale) \\ $r_j$: Number of adverse events \\ $t_j$: Strata follow-up time \\ $p_j$: Prior exchangeability probability \end{tabular}};

\node [block, node distance = 4.5cm, label={\small EX}, below left of = likelihood] (ex) {$\theta_j \sim \operatorname{Normal}(\mu_1, \tau_1)$};
\node [block, node distance = 4.5cm, label={\small NEX}, below right of = likelihood] (nex) {$\theta_j \sim \operatorname{Normal}(\mu_0, \tau_0)$};
